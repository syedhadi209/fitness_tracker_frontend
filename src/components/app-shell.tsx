    ...
    <div className="min-h-screen bg-cream">
      <aside className="fixed inset-y-0 left-0 hidden w-60 flex-col border-r border-ink/8 bg-paper px-4 py-6 lg:flex">
        <Link href="/" className="mb-8 px-2">
          ...
          <p className="text-xs text-muted">Log it like a text</p>
        </Link>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-medium transition ${
                  active ? "bg-ink text-lime" : "text-ink/70 hover:bg-sand"
                }`}
              >
                <Icon size={18} />
                {item.label}
        ...
        <div className="mt-auto flex items-center gap-3 rounded-2xl bg-sand px-3 py-3">
